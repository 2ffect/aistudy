...
def train_model(model, train_dataloader, val_dataloader, 
                num_epochs=6, learning_rate=2e-5, batch_size=8,             
                device='cuda'):
    """
    모델을 훈련합니다.
...
def main():
    ...
    # 데이터로더 생성
    train_loader = DataLoader(tokenized_dataset["train"], batch_size=8, shuffle=True, collate_fn=SmartCollator(tokenizer))
    val_loader = DataLoader(tokenized_dataset["validation"], batch_size=8, shuffle=False, collate_fn=SmartCollator(tokenizer))
    test_loader = DataLoader(tokenized_dataset["test"], batch_size=8, shuffle=False, collate_fn=SmartCollator(tokenizer))
